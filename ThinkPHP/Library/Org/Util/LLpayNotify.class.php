...
class LLpayNotify {
	...
	function verifyNotify() {
		//生成签名结果
			$is_notify = true;
			import('Org.Util.LLpayJson');
			//include_once ('llpay_cls_json.php');
			$json = new JSON;
			$str = file_get_contents("php://input");
                        //logResult($str);
			//$val =json_decode(stripslashes($str),true);
			$val = $json->decode($str);
	...
	function getSignVeryfy($para_temp, $sign) {
		...
		switch (strtoupper(trim($this->llpay_config['sign_type']))) {
			case "MD5" :
				//file_put_contents("log.txt", "MD5:" . $this->llpay_config['sign_type'] . "\n", FILE_APPEND);
				$isSgin = md5Verify($prestr, $sign, $this->llpay_config['key']);
				break;
			    case "RSA" :
			    //file_put_contents("log.txt", "RSA:" . $this->llpay_config['sign_type'] . "\n", FILE_APPEND);
				$isSgin = Rsaverify($prestr, $sign);
				break;
			default :
				//file_put_contents("log.txt", "default:" . $this->llpay_config['sign_type'] . "\n", FILE_APPEND);
				$isSgin = false;
		}
		//file_put_contents("log.txt", "加密:" . $isSgin . "\n", FILE_APPEND);
		return $isSgin;
